...
const LostPostDetail = () => {
  ...
  const renderMap = () => {
    if (lostItemData.storageLocation === "정보문화관") {
      ...
      return (
        <div className="LostPostDetail_map_message">
          보관 장소에 대한 지도가 없습니다.
        </div>
      );
    }
        ...
        {isEditing ? (
          <>
            <input
              ...
              onChange={(e) =>
                setEditedData({ ...editedData, itemType: e.target.value })
              }
              placeholder="분실물 유형을 입력하세요"
            />
            <div className="LostPostDetail_time_location">
              분실 시간 :{" "}
              {formatDate(lostItemData.lostTime) || "시간 정보 없음"}
            </div>
            <div className="LostPostDetail_time_location">
          ...
          <>
            <div className="LostPostDetail_lostitem">
              {lostItemData.itemType || "분실물"}
            </div>
            <div className="LostPostDetail_time_location">
              분실 시간 :{" "}
              {formatDate(lostItemData.lostTime) || "시간 정보 없음"}
